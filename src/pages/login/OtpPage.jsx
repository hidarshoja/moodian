...
import { checkForAdmin } from '../../utils/CheckAdmin'
...
export default function OtpPage() {
  ...
  const handleOtp = async (e) => {
    ...
    try {
      ...
      const check = checkForAdmin(response.data.data.user.roles)
      if(!check){
        errorMessage('شما اجازه ورود را ندارید')
        return
      }
      ...
      setTimeout(() => {
        setIsLoading(false)
        navigate('/',{state:{id:response.data.data.user.id}})
      }, 3100)
