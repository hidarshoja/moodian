...
export default function FactorInvoiceCompare({
  invoiceData,
  loading,
  refresh,
  setRefresh,
  ...
  const handleShowAssign = (i, r) => {
    ...
    axiosClient
      .get(
        `/invoices?page=${pageCount2}&f[type]=1&f[sum_associated_sales] = <,tadis,${r.id}`
      )
      .then((response) => {
          ...
          <tbody>
            ...
            {invoiceData.map((r, i) => (
              ...
              >
                ...
                <td className="px-4 text-center py-3 text-white/90 text-sm whitespace-nowrap">
                  {(() => {
                    const tadis = Number(r?.tadis);
                    const sum = Number(r?.sum_associated_purchases_amount);
                    ...
                    // قرمز - هیچ خریدی تخصیص داده نشده
                    if (sum === 0) {
                      return <IoCloseCircle className="text-red-500 w-5 h-5" />;
                    }
                    // سبز - تخصیص کامل شده
                    if (tadis <= sum) {
                      return (
                    ...
                    // نارنجی - تخصیص ناقص
                    return <IoMdAlert className="text-orange-500 w-5 h-5" />;
                  })()}
                </td>
                <td className="p-2  text-center"></td>
                <td
