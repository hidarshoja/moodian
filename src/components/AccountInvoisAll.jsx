...
export default function AccountInvoisAll({
  invoiceData,
  pageCount,
  setPageCount,
  setLoading,
  ...
  const handleShowAssign = (r) => {
    ...
    axiosClient
      .get(
        `/invoices?page=${pageCount2}&f[sum_transactions] = <,tadis,${r.id}&f[type]=-1`
      )
      .then((response) => {
  ...
  useEffect(() => {
    if (idActive) {
      axiosClient.get(`/transactions/${idActive}`).then((response) => {
              ...
              >
                ...
                <td className="px-4 py-3 text-center text-white/90 text-sm whitespace-nowrap">
                  {new Intl.NumberFormat("fa-IR").format(
                    r?.sum_invoices_assigned_amount
                  )}
                </td>
                <td className="px-4 py-3 text-center text-white/90 text-sm whitespace-nowrap">
                  {(() => {
                    const amount = Number(r?.amount);
                    const sum = Number(r?.sum_invoices_assigned_amount);
                    ...
                    // قرمز - هیچ فاکتوری تخصیص داده نشده
                    if (sum === 0) {
