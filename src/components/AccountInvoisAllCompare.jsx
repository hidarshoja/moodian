...
export default function AccountInvoisAllCompare({
  invoiceData,
  pageCount,
  setPageCount,
  setLoading,
  ...
  const handleShowAssign = (r) => {
    ...
    axiosClient
      .get(
        `/invoices?page=${pageCount2}&f[type]=-1&f[sum_associated_purchases] = <,tadis,${r.id}&include=sum_associated_purchases`
      )
      .then((response) => {
              ...
              >
                ...
                <td className="px-4 py-3 text-center text-white/90 text-sm whitespace-nowrap">
                  {(() => {
                    ...
                    const sum = Number(r?.sum_associated_sales_amount);
                    ...
                    // قرمز - هیچ فروشی تخصیص داده نشده
                    if (sum === 0) {
