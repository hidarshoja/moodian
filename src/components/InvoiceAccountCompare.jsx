...
export default function InvoiceAccountCompare({
  invoiceData,
  loading,
  refresh,
  setRefresh,
  ...
  useEffect(() => {
    if (idActive) {
      axiosClient.get(`/invoices/${idActive}`).then((response) => {
        const associatedSales = response?.data?.associated_sales ?? [];
        setActiveAccount(associatedSales);
        const selectedIds = associatedSales
...
InvoiceAccountCompare.propTypes = {
