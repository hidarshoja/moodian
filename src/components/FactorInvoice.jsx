...
export default function FactorInvoice({
  invoiceData,
  loading,
  refresh,
  setRefresh,
  ...
  const handleShowAssign = (i, r) => {
    ...
    axiosClient
      .get(
        `/transactions?page=${pageCount2}&f[coefficient]=1&f[sum_invoices] = <,amount,${r.id}`
      )
      .then((response) => {
              ...
              >
                ...
                <td className="px-4 py-3 text-center text-white/90 text-sm whitespace-nowrap">
                  {(() => {
                    ...
                    const sum = Number(r?.sum_transactions_assigned_amount);
                    ...
                    // قرمز - هیچ تراکنشی تخصیص داده نشده
                    if (sum === 0) {
