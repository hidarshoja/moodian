...
export default function AccountInvoisCompare({
  invoiceData,
  pageCount,
  setPageCount,
  setLoading,
  ...
  // فروش با خرید
  const fetchAssignableInvoices = useCallback(
    (targetId) => {
      if (!targetId) {
        return;
      }
      ...
      axiosClient
        .get(
          `/invoices?page=${pageCount2}&f[type]=1&f[sum_associated_sales] = <,tadis,${targetId}&include=sum_associated_sales`
        )
        .then((response) => {
  ...
  useEffect(() => {
    if (assignModalOpen && idActive) {
      fetchAssignableInvoices(idActive);
    }
  }, [assignModalOpen, idActive, fetchAssignableInvoices]);
            ...
            {invoiceData.map((r, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-white/90 text-sm flex items-center justify-center whitespace-nowrap">
                  {Number(r?.sum_associated_purchases_amount) == 0 ? (
