...
export default function PasswordForm({ onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        await axiosClient.put("change-password", payload);
        ...
        if (
          ...
          setErrorMsg("خطا در تغییر رمز عبور. لطفاً دوباره تلاش کنید.");
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="w-full rounded-xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl transition-all p-6">
      ...
      {successMsg && (
        <div className="text-green-400 text-sm mb-4">{successMsg}</div>
      )}
